...
server <- function(input, output,session) {
  ...
  rawdata <- reactive({
    ...
    if (!is.null(inFile))
    {
      data1 <- read.csv(inFile$datapath,header = T,sep = ',',na.strings = c(""," ","NA","NAN"))
      return(data1)
    }
    else
  ...
  output$testsetS1 <- renderValueBox({
    ...
    lab <- ifelse(isolate(modelType)=='Regression','Rsquared','Accuracy')
  ...
  output$txt_bestModelStat1 <- renderPrint({
    if(modelType=='Regression'){
      cat('Rsquared:',(CVres()$Rsquared[1]*100),'%')
    } else {
  ...
  output$Yplot <- renderPlot({
    ...
    if(modelType=='Regression'){
      ...
      pal <- wes_palette('GrandBudapest2',n = length(unique(dataTrain$y)),type = 'c')
...
ui <- bootstrapPage(useShinyjs(),
                    ...
                    dashboardPage(#skin = 'black',
                      dashboardHeader(disable = F,title = 'Radhi ML'
                      ),
                      dashboardSidebar(
                        sidebarMenu(
